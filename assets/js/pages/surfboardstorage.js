...
/**
 * Renders a filtered list of locations based on the "Surfboard Storage" category.
 */
async function renderSurfboardStorageLocations() {
    const container = document.getElementById('locations-list');
    if (!container) return;

    showLoader(true, 'Finding Surfboard Storage locations...');
    ...
    try {
        ...
        // Filter locations that offer the "Surfboard Storage" category
        const filteredLocations = allLocations.filter(loc =>
            loc.categories && loc.categories.some(cat => cat.name === 'Surfboard Storage')
        );

        if (filteredLocations.length === 0) {
            container.innerHTML = `<p class="no-locations-message">No locations found for Surfboard Storage.</p>`;
            return;
        }
...
export default {
    render: async () => `
        <div class="content-wrapper">
            <div class="category-hero-section surfboard-storage-banner">
                <h1>Surfboard Storage</h1>
                <p>Keep your board safe and secure near your favorite surf spots.</p>
            </div>
    ...
    `,
    afterRender: async () => {
        renderSurfboardStorageLocations();
    }
};